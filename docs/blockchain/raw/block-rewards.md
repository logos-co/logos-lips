...
| 1.2.0 | $`R_\text{block}`$ also counts the channel collateral and bonds forfeited in the block, which [Mantle](bedrock-v1.1-mantle-specification.md) 1.16.0 routes to the pending rewards pool | 2026-09-18 |
...
- $`R_\text{block}`$ denotes the total amount of Execution base fees and Permanent Storage fees, and of channel collateral and bonds forfeited in the block (see [Mantle](bedrock-v1.1-mantle-specification.md#collateral)), that are routed to the pending reward pool when the block is proposed.
...
- $`R_\text{block}`$ denotes the total amount of Execution Gas and Permanent Storage fees, and of forfeited channel collateral and bonds, routed to the rewards pool in a block. Refer to [Execution Market](execution-market.md) and [Storage Markets](storage-markets.md) for how to compute $`R_{block}`$.
