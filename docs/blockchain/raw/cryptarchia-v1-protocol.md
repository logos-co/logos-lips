...
| 1.2.5 | Noted that the body root test vectors use the channel Operations as they were before [Mantle](bedrock-v1.1-mantle-specification.md) 1.16.0 | 2026-09-18 |
...
The `ChannelWithdraw` and `ChannelTransfer` leaves below are Operations removed in Mantle 1.16.0, and the `ChannelConfig` and `ChannelInscribe` leaves use their earlier payloads. These vectors are to be regenerated from the implementation.
